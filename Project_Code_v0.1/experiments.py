...
class ValidationCheck:
    ...
    def perform_checks(self):
        ...
        for check in self.checks:
            self.results[check] = "Pass" if self.simulate_check() else "Fail"
        return self.results
...
class ExperimentApp:
    ...
    def display_validation_results(self):
        for check, result in self.validation_results.items():
            ...
            result_label = ttk.Label(self.root, text=f"{check}: {result}", background="#2E2E2E", foreground=result_color,
    ...
    def setup_experiment(self):
        ...
        experiment_label = ttk.Label(experiment_window, text="Setup Your Experiment", background="#2E2E2E", foreground="#00FF00",
                                     font=('Helvetica', 14, 'bold'))
        experiment_label.pack(pady=10)

        # Textbox for experiment details
        experiment_text = tk.Text(experiment_window, height=10, width=50, bg="#2E2E2E", fg="#FFFFFF", insertbackground="#FFFFFF",
                                  highlightbackground="#FFFFFF", highlightcolor="#FFFFFF")
        experiment_text.pack(pady=10)

        # Date picker for experiment start date
        start_date_label = ttk.Label(experiment_window, text="Experiment Start Date:", background="#2E2E2E", foreground="#00FF00",
                                     font=('Helvetica', 12, 'bold'))
        start_date_label.pack(pady=5)
        self.start_date_entry = DateEntry(experiment_window, background="#2E2E2E", foreground="#FFFFFF", borderwidth=0)
        self.start_date_entry.pack(pady=5)

        # Date picker for experiment end date
        end_date_label = ttk.Label(experiment_window, text="Experiment End Date:", background="#2E2E2E", foreground="#00FF00",
                                   font=('Helvetica', 12, 'bold'))
        end_date_label.pack(pady=5)
        self.end_date_entry = DateEntry(experiment_window, background="#2E2E2E", foreground="#FFFFFF", borderwidth=0)
        self.end_date_entry.pack(pady=5)

        # Submit button
        submit_button = ctk.CTkButton(experiment_window, text="Submit Experiment", fg_color="#2E2E2E", border_width=2,
                                      border_color="#00FF00", text_color="#00FF00", corner_radius=8, width=200, height=40,
                                      hover_color="#FFFFFF", command=lambda: self.submit_experiment(experiment_text, experiment_window))
    ...
    def submit_experiment(self, experiment_text, experiment_window):
        ...
        self.display_available_farmers(experiment_window, available_farmers, experiment_details, start_date_str, end_date_str)
    ...
    def display_available_farmers(self, experiment_window, available_farmers, experiment_details, start_date, end_date):
        ...
        farmers_listbox = tk.Listbox(available_farmers_window, bg="#2E2E2E", fg="#FFFFFF", selectbackground="#00FF00", selectforeground="#2E2E2E", selectmode=tk.MULTIPLE, font=('Helvetica', 12))
        ...
        select_button = ctk.CTkButton(available_farmers_window, text="Select Farmer", fg_color="#2E2E2E", border_width=2,
                                      border_color="#00FF00", text_color="#00FF00", corner_radius=8, width=200, height=40,
                                      hover_color="#FFFFFF", command=lambda: self.confirm_and_send_contracts(farmers_listbox, available_farmers_window, experiment_details, start_date, end_date))
        select_button.pack(pady=10)

    def confirm_and_send_contracts(self, farmers_listbox, available_farmers_window, experiment_details, start_date, end_date):
        ...
        if confirmation:
            self.send_contracts(selected_farmers, experiment_details, start_date, end_date)
            messagebox.showinfo("Contracts Sent", "Contracts have been sent to the selected farmers.")
            available_farmers_window.destroy()
            self.root.quit()
            
    def send_contracts(self, selected_farmers, experiment_details, start_date, end_date):
        for farmer in selected_farmers:
            ...
            contract_data = {
                ...
                "end_date": str(end_date),      # Convert date object to string
                "certificate_statement": contract_statement
            }
            filename = f"{farmer}_contract.json"
