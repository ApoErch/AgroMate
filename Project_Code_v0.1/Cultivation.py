...
class CultivationPage(tk.Toplevel):
    ...
    def check_field_availability(self):
        ...
        # Check if the field is available in the cultivation history
        if any(self.selected_field in key for key in cultivation_history.keys()):
            ...
            messagebox.showinfo("Unavailable", "The selected field is not available for cultivation. Please choose another field.")
            self.field_checked = True
            self.confirm_field_button.configure(state="normal")
